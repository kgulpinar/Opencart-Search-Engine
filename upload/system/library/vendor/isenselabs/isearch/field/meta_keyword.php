...
class Meta_Keyword extends Library {
    public function select($is_multilingual = false) {
        if ($is_multilingual) {
            return $this->query()
                ->select("GROUP_CONCAT(DISTINCT LOWER(s_pd.meta_keyword) SEPARATOR '|')")
    ...
    public function where($word) {
        return $this->query()->like("pd.meta_keyword", "%" . $this->db->escape($word) . "%");
    }
    ...
    public function selectSortByLength($language_id) {
        return $this->query()
            ->select("MIN(CHAR_LENGTH(s_pd.meta_keyword))")
            ->from("product_description", "s_pd")
    ...
    public function matchPhraseBeginning($full_phrase) {
        return $this->query()
            ...
            ->like("s_pd.meta_keyword", $this->db->escape($full_phrase) . "%")
            ->limit(0, 1);
    }
    ...
    public function matchAnyKeywordBeginning($keywords) {
        ...
        foreach ($keywords as $keyword) {
            $keyword_matches[] = $this->query()->like("s_pd.meta_keyword", $this->db->escape($keyword) . "%");
        }
