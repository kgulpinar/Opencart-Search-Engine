...
class Ean extends Library {
    public function select($is_multilingual = false) {
        return $this->query()->select("LOWER(p.ean)");
    }

    public function where($word) {
        return $this->query()->like("p.ean", "%" . $this->db->escape($word) . "%");
    }
    ...
    public function selectSortByLength($language_id) {
        return $this->query()->select("CHAR_LENGTH(p.ean)");
    }
    ...
    public function matchPhraseBeginning($full_phrase) {
        return $this->query()
            ...
            ->like("s_p.ean", $this->db->escape($full_phrase) . "%");
    }
    ...
    public function matchAnyKeywordBeginning($keywords) {
        ...
        foreach ($keywords as $keyword) {
            $keyword_matches[] = $this->query()->like("s_p.ean", $this->db->escape($keyword) . "%");
        }
