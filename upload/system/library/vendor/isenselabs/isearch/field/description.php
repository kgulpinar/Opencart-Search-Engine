...
class Description extends Library {
    public function select($is_multilingual = false) {
        if ($is_multilingual) {
            return $this->query()
                ->select("GROUP_CONCAT(DISTINCT LOWER(s_pd.description) SEPARATOR '|')")
    ...
    public function where($word) {
        return $this->query()->like("pd.description", "%" . $this->db->escape($word) . "%");
    }

    public function join($is_multilingual = false) {
        if ($is_multilingual) {
            return $this->query()->left_join("product_description", "pd", "pd.product_id = p.product_id");
        } else {
            return $this->query()->left_join("product_description", "pd", "pd.product_id = p.product_id AND pd.language_id='" . (int)$this->config->get('config_language_id') . "'");
        }
    }

    public function selectSortByLength($language_id) {
        return $this->query()
            ->select("MIN(CHAR_LENGTH(s_pd.description))")
            ->from("product_description", "s_pd")
    ...
    public function matchPhraseBeginning($full_phrase) {
        return $this->query()
            ...
            ->like("s_pd.description", $this->db->escape($full_phrase) . "%")
            ->limit(0, 1);
    }
    ...
    public function matchAnyKeywordBeginning($keywords) {
        ...
        foreach ($keywords as $keyword) {
            $keyword_matches[] = $this->query()->like("s_pd.description", $this->db->escape($keyword) . "%");
        }
