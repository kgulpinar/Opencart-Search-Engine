...
class Model extends Library {
    public function select($is_multilingual = false) {
        return $this->query()->select("LOWER(p.model)");
    }

    public function where($word) {
        return $this->query()->like("p.model", "%" . $this->db->escape($word) . "%");
    }
    ...
    public function selectSortByLength($language_id) {
        return $this->query()->select("CHAR_LENGTH(p.model)");
    }
    ...
    public function matchPhraseBeginning($full_phrase) {
        return $this->query()
            ...
            ->like("s_p.model", $this->db->escape($full_phrase) . "%");
    }
    ...
    public function matchAnyKeywordBeginning($keywords) {
        ...
        foreach ($keywords as $keyword) {
            $keyword_matches[] = $this->query()->like("s_p.model", $this->db->escape($keyword) . "%");
        }
